...
test_im = cv2.resize(test_im, im_size)
...
thresh_im = cv2.adaptiveThreshold(bilateral_im,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,\
...
for im in cropped_images:
    ...
    cv2.waitKey(0)
    i = i + 1
    text = text + tess.image_to_string(im, lang='eng', config='--psm 10 digits')
    print(text)

#fgh = imread('Capture.png')
#text = tess.image_to_string(fgh, lang='eng', config='--psm 10 -c tessedit_char_whitelist=0123456789')
